...
Первый запуск импортирует каталог и картинки (около 5 минут), строит SIFT-индекс (около 2 минут) и индекс ретривера с эмбеддингами DINOv2 (около 7 минут). Следующие запуски видят, что версия данных не изменилась, и завершают эти шаги за секунды. API проверки доступен на `http://localhost:8080/v1/eval/predict` — это сервис `ranking`, тот же OCR + визуальный пайплайн, что стоит за продуктовым сканером.
...
Compose закрывает путь от фото до карточки и официальный маршрут оценки. Продуктовый сканер (`/api/scans`) вызывает сервис `ranking` (`services/retrieval/app/ranking.py`): он объединяет кандидатов из `ocr-retriever` (RapidOCR, per-поле кандидаты — название, винодельня, год, сорт и т.д., см. `app/label_fields.py`) и из `retriever` (визуальный DINOv2+SIFT), резолвит один slug и решает `matched`/`not_found` по порогу уверенности. Официальный маршрут оценки (`/v1/eval/predict`) отдаёт тот же top-1. Сервис `retrieval` — отдельный, чисто визуальный SIFT/RANSAC-поиск (порт 8084): он не стоит ни за сканером, ни за оценкой и нужен только `scripts/eval_search.py`. Параметры ранжирования подобраны на 9 размеченных реальных фото и 147 искажённых эталонах каталога — это малая выборка, см. «Оценка по критериям ТЗ». Для цифрового сомелье реализованы три провайдерских адаптера, но live-качество, стоимость и русский red-team набор ещё не прогнаны с реальными ключами каждого провайдера; mock-тесты не доказывают поведение внешних моделей.

### Оценка по критериям ТЗ

`scripts/eval_ranking.py` отправляет размеченные фото в `ranking` так же, как `participant_test.sh` (по одному, поле `image`, лимит 10 с), пишет `predictions.jsonl` в формате организатора и считает метрики ТЗ: долю совпадений (фото вне каталога засчитывается, только если ответ `null`), precision/recall/F1 для top-1 и top-5, отрыв top-1 от top-2 отдельно для верных и ошибочных ответов, задержки и долю ответов в SLA 3 с. Определения — в docstring скрипта. Разметка — `configs/labeled-photos.tsv` (`image_path<TAB>expected_slug`, пустой slug — вина нет в каталоге); публичные фото организатора нужно распаковать в `data/eval`:
...
Решение `matched` принимает `app/ranking.py`: лидер должен опережать второе вино не меньше чем на `RANKING_MIN_MARGIN` (по умолчанию 0.03; при 0.06 на искажённых эталонах вдвое меньше неверных ответов — 14 против 28, — но и чуть меньше ответов). Порог задаётся в `.env` (`RANKING_MIN_MARGIN=0.03`) и применяется командой `docker compose up -d ranking` — без пересборки; `docker compose restart` переменные не перечитывает. Текущее значение видно в `version.configuration` ответа `/v1/search` (`margin0.03;…`). Что отдавать скрипту организатора при `not_found`, задаёт `RANKING_EVAL_POLICY`: `matched` (по умолчанию) — пустой slug, который скрипт записывает как `null`; `top1` — всегда лучшее вино. На размеченных реальных фото (8 в каталоге, 2 вне его; прогон 2026-09-25 после сверки с этикеткой) `matched` дал долю совпадений 1.0 и F1 1.0 при медианном отрыве верных ответов 0.147, p95 задержки 3.5 с; раньше, до сверки, на 9 фото — 0.78 и 0.83, `top1` — 0.56 и 0.63; если организатор считает `null` ошибкой и в приватном наборе нет вин вне каталога, переключите на `top1`.
...
Она проверяет, что оценочный маршрут `ranking` отдаёт top-1 продуктового (или пустой slug при `not_found` и политике `matched`), формат и HTTP-ошибки, но не правильность найденного вина. Python-тесты сервиса и оценочного скрипта (тесты с БД создают и удаляют временную базу `vinolog_test_*`, нужен запущенный `db`):
