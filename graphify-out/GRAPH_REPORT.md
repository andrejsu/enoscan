...
## Corpus Check
- 175 files · ~318,416 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1456 nodes · 2761 edges · 104 communities (93 shown, 11 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 341 edges (avg confidence: 0.71)
...
- [[_COMMUNITY_fingerprint_inputs|fingerprint_inputs]]
...
1. `Wine` - 48 edges
2. `RetrievalFields` - 48 edges
3. `FieldCandidate` - 40 edges
4. `rank()` - 39 edges
5. `FieldVocabulary` - 24 edges
...
Cohesion: 0.08
Nodes (47): clean_mask(), Config, contour_points(), debug_geometry(), debug_segmentation(), _edge_bins(), ensure_sam(), estimate_geometry() (+39 more)
...
### Community 9 - "main.py"
Cohesion: 0.14
Nodes (18): Candidate, SearchResult, Candidate, ndarray, SearchResult, SiftIndex, lifespan(), FastAPI (+10 more)

### Community 11 - "catalog.py"
Cohesion: 0.17
Nodes (22): ArchiveReader, Issue, decoded_name(), is_system_entry(), ObjectStore, Path, upload_eval_sets(), main() (+14 more)
...
Cohesion: 0.15
Nodes (27): main(), run_eval(), main(), current_dataset_version(), load_references(), Reference, load_settings(), Settings (+19 more)

### Community 14 - "ocr_retriever.py"
Cohesion: 0.06
Nodes (64): Pattern, load_wines(), extract_label(), fold_homoglyphs(), load_engine(), OcrResult, OcrWord, ndarray (+56 more)
...
Cohesion: 0.11
Nodes (23): LabelConfig, Dinov2Encoder, ndarray, Path, Frozen DINOv2 global descriptor for wine-label instance retrieval.  Global embed, Read catalog art with transparent pixels composited onto white., read_embedding_image(), DINOv2 reference embeddings stored in PostgreSQL (pgvector).  Embeddings are key (+15 more)

### Community 19 - "ranking_main.py"
Cohesion: 0.05
Nodes (101): _optional(), Wine, FieldCandidate, fields_from_json(), Shared evidence struct for the OCR block and the visual retriever.  Both sources, Inverse of dataclasses.asdict(RetrievalFields) — how the OCR service     (app/oc, RetrievalFields, extract_year() (+93 more)

### Community 20 - "OcrRetriever"
Cohesion: 0.14
Nodes (18): decode_upload(), ndarray, UploadFile, read_image_upload(), decode_image(), decode_reference(), extract_features(), ndarray (+10 more)
...
Cohesion: 0.11
Nodes (19): 10. `services/retrieval/app/config.py`, 11. Удалить `services/retrieval/app/catalog_browser.py`, 12. `compose.yaml`, 13. Тесты, 1. `services/retrieval/requirements.txt`, 2. `services/retrieval/app/storage.py`, 3. `services/retrieval/app/catalog.py`, 4. `services/retrieval/app/index_store.py` (+11 more)
...
Cohesion: 0.14
Nodes (16): main(), Build the retriever's own index — separate object, separate process from app/bui, Pre-fetch the SAM segmentation model used by label_normalize's query path.  Segm, Local wine-label retrieval service., decode_reference_unchanged(), _fast_crop(), prepare_query(), prepare_reference_image() (+8 more)
...
Cohesion: 0.20
Nodes (8): EmbeddingStore, Connection, ndarray, populated(), ndarray, test_missing_is_scoped_to_model(), test_nearest_and_scores_match_numpy(), unit()
...
Cohesion: 0.33
Nodes (9): load_database_url(), load_max_upload_bytes(), _eval_policy(), load_ranking_settings(), RankingSettings, Settings for the standalone ranking service (app/ranking_main.py).  Where to rea, load_retriever_settings(), Settings for the standalone retriever service (app/retriever_main.py).  Delibera (+1 more)
...
Cohesion: 0.20
Nodes (12): catalog_sugar_level(), contradicts(), Sugar level of a wine («сухое», «полусладкое», «брют»…), from catalog text or a, Every sugar level the text names., The one sugar level the text names; None when it names none or two     that disa, The first source that names exactly one sugar level: name, then slug, then photo, Different levels, except within the brut family: OCR may read «BRUT» of «EXTRA B, sugar_level() (+4 more)
...
### Community 54 - "ObjectStore"
Cohesion: 0.22
Nodes (4): ClientError, ObjectStore, Path, _status()
...
### Community 72 - "apply_migrations"
Cohesion: 0.40
Nodes (8): apply_migrations(), migration_files(), Path, Path, test_applies_files_in_name_order_once(), test_failed_migration_is_not_recorded(), test_refuses_changed_migration(), test_repository_schema_applies()
...
- **Why does `Config` connect `label_prep.py` to `label_normalize.py`, `EmbeddingStore`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Why does `Wine` connect `ranking_main.py` to `main.py`, `index_store.py`, `ocr_retriever.py`, `Wine`, `ImageFeatures`, `sweetness.py`, `test_run.py`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Why does `ImageDecodeError` connect `images.py` to `main.py`, `catalog.py`, `ObjectStore`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
...
- **Are the 30 inferred relationships involving `RetrievalFields` (e.g. with `FieldContribution` and `LabelCheck`) actually correct?**
  _`RetrievalFields` has 30 INFERRED edges - model-reasoned connections that need verification._
- **Are the 25 inferred relationships involving `FieldCandidate` (e.g. with `FieldContribution` and `LabelCheck`) actually correct?**
  _`FieldCandidate` has 25 INFERRED edges - model-reasoned connections that need verification._
- **Are the 26 inferred relationships involving `rank()` (e.g. with `test_patterned_label_reads_the_winery_but_winery_alone_never_matches()` and `test_readable_label_yields_the_wine_name()`) actually correct?**
  _`rank()` has 26 INFERRED edges - model-reasoned connections that need verification._